...
// // use Database\MareiObj;
// // use Database\MareiCollection;
...
// echo $this->db_config['env'];die('s');
		...
		try {
			...
			$this->dbh = new \PDO('sqlite:Database/projectmanager.db'); // For SQLITE
	...
	public function query($query, $args = [], $quick = false)
	{
		...
		if ($quick == true) {
			...
			if (strpos( strtoupper($query), "SELECT" ) === 0 ) {
				...
				$rows = $stmt->fetchAll(\PDO::FETCH_CLASS,'Database\MareiObj');
				$collection= [];
